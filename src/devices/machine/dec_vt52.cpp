...
/****************************************************************************
    ...
    DECscope, but quickly displaced it by supporting 24 lines of text and a
    full ASCII character generator on a board of its own.

    The terminal is a device so that it can sit on any RS-232 port as the
    vt52 option; the vt52 driver in dec/vt52.cpp only gives it an EIA port.
...
DEFINE_DEVICE_TYPE(DEC_VT52, dec_vt52_device, "dec_vt52", "DEC VT52 Video Display Terminal")

dec_vt52_device::dec_vt52_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock)
	: device_t(mconfig, DEC_VT52, tag, owner, clock)
...
void dec_vt52_device::device_start()
{
...
void dec_vt52_device::device_reset()
{
	m_110_baud_counter = 0;
...
void dec_vt52_device::update_serial_settings()
{
...
INPUT_CHANGED_MEMBER(dec_vt52_device::data_sw_changed)
{
	update_serial_settings();
}

u8 dec_vt52_device::key_r(offs_t offset)
{
...
void dec_vt52_device::baud_9600_w(int state)
{
...
void dec_vt52_device::vert_count_w(u8 data)
{
	u16 baud = m_baud_sw->read();
...
void dec_vt52_device::uart_xd_w(u8 data)
{
...
void dec_vt52_device::gated_serial_output(bool state)
{
...
void dec_vt52_device::serial_out_w(int state)
{
...
void dec_vt52_device::break_w(int state)
{
	if (m_serial_out)
		gated_serial_output(state);
}

void dec_vt52_device::serial_in_w(int state)
{
	m_rec_data = state;
...
int dec_vt52_device::xrdy_eoc_r()
{
	return m_uart->tbmt_r() && m_uart->eoc_r();
}

u8 dec_vt52_device::chargen_r(offs_t offset)
{
	// ROM is on its own board, shared only with 7404 inverters
	return ~m_chargen[offset];
}

void dec_vt52_device::rom_1k(address_map &map)
{
	map(00000, 01777).rom().region("program", 0);
}

void dec_vt52_device::ram_2k(address_map &map)
{
	map(00000, 03777).ram();
}

static INPUT_PORTS_START(dec_vt52)
	PORT_START("KEY0")
	...
	PORT_BIT(1, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("Break") PORT_CODE(KEYCODE_PAUSE) PORT_WRITE_LINE_MEMBER(FUNC(dec_vt52_device::break_w)) // S16
	...
	PORT_DIPNAME(0x03f1, 0x01f1, "Transmitting Speed") PORT_DIPLOCATION("S1:7,4,5,6,2,3,1") PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(dec_vt52_device::data_sw_changed), 0)
	...
	PORT_DIPNAME(0x3c0e, 0x1c0e, "Receiving Speed") PORT_DIPLOCATION("S2:6,5,4,2,1,3,7") PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(dec_vt52_device::data_sw_changed), 0)
	...
	PORT_START("DATABITS")
	PORT_DIPNAME(0x1, 0x1, "Data Bits") PORT_DIPLOCATION("S3:1") PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(dec_vt52_device::data_sw_changed), 0)
	PORT_DIPSETTING(0x0, "7 (with parity)")
	PORT_DIPSETTING(0x1, "8 (no parity)")
	PORT_DIPNAME(0x2, 0x2, "Parity") PORT_DIPLOCATION("W6:1") PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(dec_vt52_device::data_sw_changed), 0)
	PORT_DIPSETTING(0x2, "Even")
	PORT_DIPSETTING(0x0, "Odd")
	PORT_DIPNAME(0x4, 0x0, "Data Bit 7") PORT_DIPLOCATION("W5:1") PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(dec_vt52_device::data_sw_changed), 0)
...
ioport_constructor dec_vt52_device::device_input_ports() const
{
	return INPUT_PORTS_NAME(dec_vt52);
}

void dec_vt52_device::device_add_mconfig(machine_config &config)
{
	VT52_CPU(config, m_maincpu, 13.824_MHz_XTAL);
	m_maincpu->set_addrmap(AS_PROGRAM, &dec_vt52_device::rom_1k);
	m_maincpu->set_addrmap(AS_DATA, &dec_vt52_device::ram_2k);
	m_maincpu->set_screen("screen");
	m_maincpu->baud_9600_callback().set(FUNC(dec_vt52_device::baud_9600_w));
	m_maincpu->vert_count_callback().set(FUNC(dec_vt52_device::vert_count_w));
	m_maincpu->uart_rd_callback().set(m_uart, FUNC(ay51013_device::receive));
	m_maincpu->uart_xd_callback().set(FUNC(dec_vt52_device::uart_xd_w));
	m_maincpu->ur_flag_callback().set(m_uart, FUNC(ay51013_device::dav_r));
	m_maincpu->ut_flag_callback().set(FUNC(dec_vt52_device::xrdy_eoc_r));
	m_maincpu->ruf_callback().set(m_uart, FUNC(ay51013_device::write_rdav));
	m_maincpu->key_up_callback().set(FUNC(dec_vt52_device::key_r));
	...
	m_maincpu->char_data_callback().set(FUNC(dec_vt52_device::chargen_r));

	AY51013(config, m_uart); // TR1402 or equivalent
	m_uart->write_so_callback().set(FUNC(dec_vt52_device::serial_out_w));
...
const tiny_rom_entry *dec_vt52_device::device_rom_region() const
{
	return ROM_NAME(dec_vt52);
}
