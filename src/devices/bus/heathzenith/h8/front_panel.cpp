...
// Input ports
//
// These are IPT_KEYPAD rather than IPT_KEYBOARD, and carry no PORT_CHAR.  The
// front panel is an octal keypad - its keys stand for digits and monitor
// functions, not for characters - and natural keyboard builds its character map
// from IPT_KEYBOARD fields alone, so a keypad claims nothing.  That matters on a
// machine that also has a terminal: as IPT_KEYBOARD with a PORT_CHAR on every
// key, the panel sorts first by tag, takes "the first keyboard" slot that
// natural_keyboard enables by default, and swallows every digit, "-" and "="
// meant for the console - a date typed as 02-JUL-77 arrives as JUL.  Two of the
// characters were arbitrary anyway ('Q' for * CANCEL, 'R' for . REG).  Keys
// still work normally in emulated keyboard mode through their PORT_CODE.
static INPUT_PORTS_START( h8 )
	PORT_START("X0")
	PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("0")           PORT_CODE(KEYCODE_0)      PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(front_panel_device::button_0), 0)
	PORT_BIT( 0x02, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("1 SP")        PORT_CODE(KEYCODE_1)
	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("2 AF")        PORT_CODE(KEYCODE_2)
	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("3 BC")        PORT_CODE(KEYCODE_3)
	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("4 DE GO")     PORT_CODE(KEYCODE_4)
	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("5 HL IN")     PORT_CODE(KEYCODE_5)
	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("6 PC OUT")    PORT_CODE(KEYCODE_6)
	PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("7 SI")        PORT_CODE(KEYCODE_7)

	PORT_START("X1")
	PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("8 LOAD")      PORT_CODE(KEYCODE_8)
	PORT_BIT( 0x02, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("9 DUMP")      PORT_CODE(KEYCODE_9)
	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("+")           PORT_CODE(KEYCODE_UP)
	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("-")           PORT_CODE(KEYCODE_DOWN)
	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("* CANCEL")    PORT_CODE(KEYCODE_ESC)
	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("/ ALTER RST") PORT_CODE(KEYCODE_EQUALS)
	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("# MEM RTM")   PORT_CODE(KEYCODE_MINUS)
	PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME(". REG")       PORT_CODE(KEYCODE_R)
INPUT_PORTS_END
